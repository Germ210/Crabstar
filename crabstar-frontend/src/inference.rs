use crate::{
    ...
    types::{
        ...
        int_type, int32_type, mut_type, null_type, owned_type, ref_type, string_type,
    },
};
use rowan::GreenNode;
...
impl Inferencer {
    ...
    pub fn infer(&mut self, ast: Expr) -> InferenceResult {
        let inference_res = match ast {
            ...
            _ => todo!("Implement type inference for type: {:#?}", ast),
        };

        self.node_types
            .insert(ast.syntax().clone(), inference_res.ty);
        self.inference_results
            .insert(ast.syntax().green().into_owned(), inference_res.clone());
        inference_res
    }
                ...
                match pattern {
                        ...
                        if let Some(binding_list) = variant.binding_list() {
                            ...
                            if bindings.len() == 1 {
                                if let Some(Pattern::Ident(ident)) = bindings[0].pattern() {
                                    let name =
                                        unwrap_or_err!(ident.name(), self).text().to_string();
                                    if let Some(bound_ty) = branch_assumptions.remove(&name) {
                                        let (err, _) = self.unify(
                                            bound_ty,
                                            payload_ty,
                    ...
                    Pattern::DestructureStruct(destructure) => {
                        ...
                        for field in destructure.fields() {
                            let binding_name =
                                unwrap_or_err!(field.name(), self).text().to_string();
                            let field_expr = unwrap_or_err!(field.value(), self);
                            let field_name =
                                field_expr.syntax().text().to_string().trim().to_string();
                            let field_ty =
                                if let Some(bound_ty) = branch_assumptions.remove(&binding_name) {
                                    bound_ty
                                } else {
                                    self.fresh_type_var()
                                };
    ...
    pub fn unify(
        ...
        match (a_ty, b_ty) {
            ...
            (Type::TypeApp(app), _) if app.args.is_empty() => {
                self.unify(app.head, b, a_node, b_node)
            }
            (_, Type::TypeApp(app)) if app.args.is_empty() => {
                self.unify(a, app.head, a_node, b_node)
            }
            ...
            (Type::Row(a_row), Type::Row(b_row)) => {
                ...
                for (name, a_ty) in &a_fields {
                    if let Some(b_ty) = b_fields.get(name) {
                        if let (Some(err), _) =
                            self.unify(*a_ty, *b_ty, a_node.clone(), b_node.clone())
                        {
                            return (Some(err), self.new_err());
                        }
                    }
